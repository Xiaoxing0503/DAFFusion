...
class DynamicFilter(nn.Module):
    ...
    def forward(self, x, y):
        ...
        routeing = self.reweight(y.mean(dim=(1, 2))).view(B, self.num_filters, -1).softmax(dim=1) #(B, num_filters, med_channels)
 
        # 特征扩展：点全连接 + 激活
        x = self.pwconv1(x)
        x = self.act1(x) #(B, H, W, med_channels)
 
        # 转换到频域（二维实值快速傅里叶变换）
        x = x.to(torch.float32)
        x = torch.fft.rfft2(x, dim=(1, 2), norm='ortho') # 结果是复数张量 (B, H, W//2 + 1, med_channels)
        ...
        if self.weight_resize:
            ...
            complex_weights = torch.view_as_complex(complex_weights.contiguous()) # 转换为复数张量 (H, W//2 + 1, num_filters)
        else:
        ...
        weight = torch.einsum('bfc,hwf->bhwc', routeing, complex_weights) # 爱因斯坦求和实现加权 (B, H, W//2+1, med_channels)
 
        # 调整权重维度以匹配特征
        if self.weight_resize:
            weight = weight.view(-1, x.shape[1], x.shape[2], self.med_channels) # (B, H, W//2+1, med_channels)
        else:
            weight = weight.view(-1, self.size, self.filter_size, self.med_channels)
 
        # 频域特征与滤波器相乘（核心操作）
        x = (x-y) * weight # (B, H, W//2+1, med_channels)
 
        # 转换回空间域（逆快速傅里叶变换）
        x = torch.fft.irfft2(x, s=(H, W), dim=(1, 2), norm='ortho')  # (B, H, W, med_channels)
